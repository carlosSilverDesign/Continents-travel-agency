...
export default async function TourDetail({ params }: Props) {
  ...
  const mensaje = t('whatsappMessage', { title: tour.title, price: tour.price });
  ...
  return (
    <main className="min-h-screen bg-ui-bg pb-20">
        ...
        <div className="absolute inset-0 bg-black/50 flex items-end">
          <div className="max-w-7xl mx-auto w-full px-4 md:px-8 pb-10">
            ...
            <p className="text-xl text-white/90">
              {tour.duration_days} {t('days')} {tour.duration_nights ? `/ ${tour.duration_nights} ${t('nights')}` : ''}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto w-full px-4 md:px-8 py-10 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 bg-ui-surface p-6 md:p-10 rounded-2xl shadow-sm border border-ui-border">
          <TourTabs
        ...
        <div className="w-full lg:w-[350px]">
          <div className="bg-ui-surface p-6 rounded-2xl shadow-sm border border-ui-border sticky top-24">
            <p className="text-sm text-ui-text uppercase tracking-wide font-semibold mb-2">{t('pricePerPerson')}</p>
            <div className="flex items-end gap-2 mb-2">
              <span className="text-4xl font-bold text-primary">US$ {tour.price}</span>
            </div>
            <p className="text-xs text-ui-text mb-6">
              {tour.price_disclaimer || t('basePriceDisclaimer')}
            </p>
